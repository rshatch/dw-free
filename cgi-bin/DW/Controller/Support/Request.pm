...
sub see_request_handler {
    ...
    $vars->{spid} = $spid;
        ...
        if ($foundspid) {
            ...
            my $text = $find eq 'next' || $find eq 'cnext' ? LJ::Lang::ml('.error.nonext' . $extra) : LJ::Lang::ml('.error.noprev' . $extra);
            my $goback = $sp ? LJ::Land::mln hour('.goback.text', {request_link=>"href='see_request?id=$spid'", spid=>$spid}) : undef;
    ...
    my $display_name;
    {
        ...
        if ($show_history{user} || $show_history{email}) {
            $display_name= $sp->{reqtype} eq 'user' && $show_history{user} ?
    ...
    $vars->{accounttype} = LJ::Capabilities::name_caps( $u->{caps} ) || "<i>" . LJ::Lang::ml('/support/see_request.tt.unknown') . "</i>";
    ...
    if ($state eq "open") {
        ...
        if ($sp->{'timelasthelp'} > ($sp->{'timetouched'}+5)) {
            ...
            # default
            $state = "<b><span style='color: #ff0000;'>" . LJ::Lang::ml('/support/see_request.tt.open') . "</span></b>";
        }
    }
        ...
        if ($can_close || $can_reopen) {
            if ($sp->{'state'} eq "open" && $can_close) {
                $state .= ", <a href='act?close;$sp->{'spid'};$sp->{'authcode'}'><b>".LJ::Lang::ml('/support/see_request.tt.close.without.credit')."</b></a>";
            }
            elsif ($sp->{state} eq 'closed') {
                my $permastatus = LJ::Support::is_locked($sp);
                $state .= $sp->{'state'} eq "closed" && !$permastatus ?
                    ", <a href='act?touch;$sp->{'spid'};$sp->{'authcode'}'><b>" . LJ::Lang::ml('/support/see_request.tt.reopen.this.request')."</b></a>" : "";
                if (LJ::Support::can_lock($sp, $remote)) {
                    $state .= $permastatus ?
                        ", <a href='act?unlock;$sp->{spid};$sp->{authcode}'><b>" . LJ::Lang::ml('/support/see_request.tt.unlock.request')."</b></a>" :
                        ", <a href='act?lock;$sp->{spid};$sp->{authcode}'><b>" .LJ::Lang::ml('/support/see_request.tt.lock.request')."</b></a>";
                }
            }
        }
            $vars->{state} = $state;
            ...
            my $extraval = sub {
                ...
                return " (<a href='$LJ::SITEROOT/register?foruser=$u->{user}'>".LJ::Lang::ml('/support/see_request.tt.resend.validation.email')."</a>)";
            };

            my $email_status;
            if ($u->{'status'} eq "A") {$email_status .= "<b>". LJ::Lang::ml('/support/see_request.tt.yes')."</b>";}
            if ($u->{'status'} eq "N") {$email_status = "<b>". LJ::Lang::ml('/support/see_request.tt.no')."</b>" . $extraval->();}
            if ($u->{'status'} eq "T") {$email_status = LJ::Lang::ml('/support/see_request.tt.transitioning') . $extraval->();}
            ...
            foreach my $le (@replies) {
                ...
                if ($le->{'type'} eq "req") {
                    ...
                    $message .= "<hr><strong>".LJ::Lang::ml('/support/see_request.tt.diagnostics')."</strong> " . LJ::ehtml($props->{useragent}) if $props->{useragent};
                    $reply->{msg} = $message;
                    $reply->{orig} = 1;
                    next;
                }
            ...
            $vars->{show_note} = !(LJ::Support::can_read_internal($sp, $remote) and ($ans_type{'answer'} || $ans_type{'screened'}));
            ...
            if ($stock_mode && !$is_poster) {
                ...
                if ($rows && @$rows) {
                    ...
                    $stocks_html .= "<option value='-1' selected>( ".LJ::Lang::ml('/support/see_request.tt.select.canned.to.insert')." )</option>\n";
                    $i = 0;
                    foreach my $row (@$rows) {
            ...
            $vars->{userfacing_actions_list} = [ map {$ans_type{$_} ? ($_ => $ans_type{$_}) : ()}
                qw(screened answer comment) ];
            $vars->{internal_actions_list} = [ map {$ans_type{$_} ? ($_ => $ans_type{$_}) : ()}
                qw(internal bounce) ];
