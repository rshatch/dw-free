...
sub changenotify_handler {
    ...
    if ($mode eq "modify") {
        my %notify;
        my $dbr = LJ::get_db_reader();
        my $sth = $dbr->prepare("SELECT spcatid, level FROM supportnotify WHERE userid=$remote->{'userid'}");
        $sth->execute;
        ...
        my %valname = ("off" => LJ::Lang::ml('.option.off'),
...
sub stock_answer_handler {
    ...
    if ($GET->{new}) {
        my @options = map {$_, $cats->{$_}->{catname}}
            grep {$canview{$_}}
                sort {$cats->{$a}->{catname} cmp $cats->{$b}->{catname}}
                    keys %$cats;
        unshift @options, 0, "( please select )";
